...
export const getUrlHistory = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Ensure we only get current user's URLs
    const urls = await Url.find({ userId }).sort({ submittedAt: -1 });

    console.log('🔍 DEBUG - getUrlHistory found:', urls.length, 'URLs for user:', userId);
...
export const getUserStats = async (req, res) => {
  try {
    ...
    // ✅ ADD DEBUG LOGS TO VERIFY USER ID
    console.log('🔍 DEBUG - getUserStats user ID:', userId);
    console.log('🔍 DEBUG - Authenticated user:', req.user.email);

    // ✅ FIXED: Ensure all queries filter by userId
    const totalSubmitted = await Url.countDocuments({ userId });
    const totalIndexed = await Url.countDocuments({ 
      userId, 
      status: 'indexed' 
    });
    ...
    // ✅ ADD DEBUG COUNTS TO VERIFY
    const allUsersCount = await Url.countDocuments({});
    console.log('🔍 DEBUG - URL counts - All users:', allUsersCount, 'Current user:', totalSubmitted);
